The first question that comes to mind is "Why to learn CLI/Terminal when GUI exists?". There are multiple reasons for this.
- Greater Control - While GUI does make things easier to navigate and does not require us to remember commands, learning Command Line/Terminal is sort of like having backend access to the system so that we can do things that we typically cannot do using the Graphical Interface.
- Speed & Efficiency - It may sound dubious but learning to navigate the system using commands allows us to do a lot of things more quickly and easily like dealing with multiple or hundreds of files that would take us days to manually handle on the GUI. Instead using CLI, we can do tasks very quickly using certain commands and without having to go back and forth between the keyboard and the mouse.
- Access Remote Servers - Using CLI, we can access remote servers like SSH. Also, some operating systems do not have a GUI. Such OS are called Headless OS.
- Command Line Tools - Nowadays, all our packages and frameworks are installed using npm or node package manager.

Usually, the term 'Command Line' is very Window centric while Terminal is very Mac centric. But we will discuss both.

- Terminal - Tool that we use to type and execute commands. Back in the day, it used to be a separate device with a screen that could display text and get input. It was usually connected to some big Mainframe or early Unix-based system. Now, we use a "terminal-emulator" software.
- Command Line - The interface of the terminal. Windows often call their terminal program the Command Line. The two terms are used interchangeably. Command line or cmd (in Windows) is where we write the commands or give input.
- The Shell - The program that the terminal runs. The "OS" of the terminal. Reads the input and displays the output. Bash was the most commonly used shell for many Linux and Unix systems. And up until recently it was the default shell for MacOS but now it has been replaced with 'ZSH'.

Let's talk about Windows. Windows used to use a shell called 'COMMAND.COM' that was later switched to 'CMD.EXE'. It is very limiting and based on MS-DOS style commands. Windows also offers Powershell (modern shell by Microsoft; more powerful than cmd), which has different commands (some of them same) than bash, ZSH and other Unix-based Shells. There are a lot of Unix-based solutions for Windows like Git Bash and WSL (Windows Subsystems for Linux).
